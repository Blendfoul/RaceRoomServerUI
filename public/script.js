const fetchServerData = async (target) => {
...
const fillTemplateData = async (data, tracks, key) => {
...
const render = async (region = "All") => {
    try {
        const serverData = await fetchServerData(`/server/${region}`);
        serverData.result.sort((a, b) => b.Server.PlayersOnServer - a.Server.PlayersOnServer);
        const tracks = await fetchServerData("/tracks");
        serverData.result.map(async (data, key) => await fillTemplateData(data, tracks, key));
    }
    catch (err) {
        console.error(err);
    }
